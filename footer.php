...
if( empty($dabble_option['enable_global'])) {
    ...
    <?php elseif( !empty( $dabble_option['footer_bg_image']['url'])):?>
        <footer id="rs-footer" class="<?php echo esc_attr($footer_select);?> rs-footer footer-style-1" style="background-image: url('<?php echo esc_url($dabble_option['footer_bg_image']['url']);?>'); background-position: <?php echo esc_attr($footer_bg_pos); ?>">
        <?php else:?>
            <footer id="rs-footer" class="<?php echo esc_attr($footer_select);?> rs-footer footer-style-1" >
    <?php endif;
    } else {
       if( !empty( $dabble_option['footer_bg_image']['url'])):?>
        <footer id="rs-footer" class="<?php echo esc_attr($footer_select);?> rs-footer footer-style-1" style="background-image: url('<?php echo esc_url($dabble_option['footer_bg_image']['url']);?>'); background-position: <?php echo esc_attr($footer_bg_pos); ?>">
        <?php else:?>
            <footer id="rs-footer" class="<?php echo esc_attr($footer_select);?> rs-footer footer-style-1" >
    <?php endif;
            ...
            <?php if(is_active_sidebar( 'copy_right'  )): ?>                      
                <div class="col-md-6 col-sm-12 copy1">
                    <div class="copyright text-left" <?php if(!empty( $copy_space)): ?> style="padding: <?php echo esc_attr($copy_space); ?>" <?php endif; ?> >
                        <?php if(!empty($dabble_option['copyright'])){?>
                        <p><?php echo wp_kses($dabble_option['copyright'], 'dabble'); ?></p>
                        <?php }
                         else{
                            ?>
...
<?php 
if(!empty($dabble_option['show_top_bottom'])){
?>
 <!-- start scrollUp  -->
<div id="scrollUp">
